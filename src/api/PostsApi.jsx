import axios from "axios";


const apiKey = "05646635804321276";

const token =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..DbHEF0OBbliIKdL32lCzmqLItiGsF_W7a5m0d3N-kG0";

export const addRole = async (roleData) => {
  const response = await axios.post(
    "https://green-bill-api-dev.myjilo.com/api/role",
    roleData,
    {
      headers: {
        "x-api-key": apiKey,
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    }
  );
  return response.data;
};

export const getRoles = (filtersa = {}) => {
  const defaultFilters = {
    where: {
      isActive: true,
      isDeleted: false,
      searchText: "",
    },
    pagination: {
      sortBy: "createdAt",
      descending: true,
      rows: 100,
      page: 1,
    },
  };

  const finalFilters = {
    ...defaultFilters,
    ...filtersa,
    where: {
      ...defaultFilters.where,
      ...(filtersa.where || {}),
    },
    pagination: {
      ...defaultFilters.pagination,
      ...(filtersa.pagination || {}),
    },
  };

  return axios.post(
    "https://green-bill-api-dev.myjilo.com/api/role/list",
    finalFilters, 
    {
      headers: {
        "x-api-key": apiKey,
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    }
  );
};


export const deleteRole  = async (id) => {
  try {
    const url = `https://green-bill-api-dev.myjilo.com/api/role/${id}`;
    console.log("Deleting URL:", url);

    const res = await axios.delete(url, {
      headers: {
        "x-api-key": apiKey,
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
    });
    return res;
  } catch (error) {
    console.error("Error while deleting:", error.response || error.message);
    throw error;
  }
};

export const updateRole = async (id, post) => {
  try {
    const res = await axios.put(
      `https://green-bill-api-dev.myjilo.com/api/role/${id}`, 
      post, 
      {
        headers: {
          "x-api-key": apiKey,
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
        },
      }
    );
    console.log("Role updated:", res.data);
    return res;
  } catch (error) {
    console.error("PUT error:", error.response?.data || error.message);
    return error.response;
  }
};

